...
class DataModule(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        ...
        val_dataloader = DataLoader(
            ...
            drop_last= False, # use all available data for validation
            worker_init_fn=worker_init_fn,
            prefetch_factor=2,
            persistent_workers=True,
        )
...
if __name__ == "__main__":
    ...
    print("[DBG] val_dataloader type:", type(vdl))

    # If it's a single DataLoader:
    try:
        n = len(vdl)
        print("[DBG] #val batches:", n)
        first = next(iter(vdl))
        print("[DBG] sample val batch keys:", list(first.keys()) if isinstance(first, dict) else type(first))
    except TypeError:
        print("[DBG] val_dataloader has no __len__; iterating one batch…")
        try:
            first = next(iter(vdl))
            print("[DBG] got a batch")
        except StopIteration:
            print("[DBG] val_dataloader is EMPTY (no batches)")
